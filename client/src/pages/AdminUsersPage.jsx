    ...
    <section className="space-y-4 p-4 lg:p-5">
      ...
      <section className="overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm">
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-background"><tr><th className="p-3">Name</th><th>Email</th><th>Role</th><th>Status</th><th>Last Login</th></tr></thead>
          <tbody>
            {users.map((user) => <tr key={user.id} className="border-t border-black/10"><td className="p-3 font-medium">{user.name}</td><td>{user.email}</td><td className="capitalize">{user.role?.replace("_", " ")}</td><td><StatusBadge status={user.status || "ACTIVE"} /></td><td>{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString() : "-"}</td></tr>)}
            {!users.length && <tr><td colSpan={5} className="p-6 text-center text-sm text-black/50">No users returned by the backend.</td></tr>}
          </tbody>
        </table>
      </section>
