    ...
    <section className="rounded-lg border border-black/10 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid gap-5 sm:grid-cols-[11rem_1fr]">
        ...
        <div className="text-sm">
          ...
          {data?.drivers?.length ? data.drivers.map((driver) => {
          ...
          }) : <p className="mt-2 text-black/50">No forecast drivers returned by the backend.</p>}
          <p className="mt-3 font-bold text-emerald-700">Recommendation</p>
          <ul className="mt-1 space-y-1 text-black/65">
            {data?.recommendation?.length ? data.recommendation.map((item) => <li key={item}>{item}</li>) : <li>No recommendations returned.</li>}
          </ul>
        </div>
      </div>
    </section>
